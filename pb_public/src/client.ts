...
import { AmbientLight, DirectionalLight, DirectionalLightHelper, MeshStandardMaterial, PerspectiveCamera, Raycaster, Scene, Vector2, WebGLRenderer } from "three";
...
import { MsgJson, QueryRoom, PlayerState, MsgHandler } from "./api.js";
...
export async function client_start (ui: UIBuilder, container: HTMLDivElement, penguinConfig: PenguinConfig) {


  //create a canvas
  let canvas = ui.create("canvas").id("canvas").mount(container).e as HTMLCanvasElement;
  ...
  let penguins = new Map<number, Penguin>();

  function removePenguin (id: number) {
  ...
  msgHandler.listen("player-init-resp", async (sender: WebSocket, json) => {
    ...
    localPenguin.setTarget(
      lerp(-spawnRadius, spawnRadius, Math.random()),
      0,
      lerp(-spawnRadius, spawnRadius, Math.random())
    );
    ...
    } as MsgJson<QueryRoom>));

  });

  async function handlePlayerState (sender: WebSocket, data: PlayerState, teleport: boolean = false) {
  ...
  msgHandler.listen("state", async (sender: WebSocket, json) => {
    handlePlayerState(sender, json.data);
  });

  msgHandler.listen("query-room-resp", (sender: WebSocket, json)=>{
  ...
  msgHandler.listen("room-change", (sender: WebSocket, json)=>{
  ...
  ui.ref(window as any).on("resize", resize);
